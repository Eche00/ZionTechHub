...
function Topheader() {
  ...
  const bannerMessage = (
    <p className="md:text-[18px] lg:text-[14px] text-[12px] text-[#FFFFFF] font-[300] p-[16px] bg-[#1A1A1A] w-full text-center">
      ...
      <Link to="/zth-partnership-program" className="font-[600] underline px-5">
        Join Now
      </Link>
    </p>
  );
  ...
  // proper blog route handling (THIS IS THE IMPORTANT FIX)
  const isBlogRoute =
  ...
  ].includes(pathname) || isBlogRoute;

  return (
    <div>
      {/* General pages + blog + blog slug */}
      {isGeneralPage && (
        <>
          {bannerMessage}
          <Navbar />
